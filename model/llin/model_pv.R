...
config$simulate <- function(theta){
  ...
  ## beta
  beta0 <- ifelse(config$tr_template$period_llin == 2, theta[["beta0_1"]], theta[["beta0_0"]])
  ...
  ## mu_HI
  mu_HI <- 1/config$fixed_param$T_HI
  
  # mu_HS
  mu_HS <- 1/config$fixed_param$T_HS
  ...
  # H0 
  lambda0 <- beta[1]*(IA0/kappa+IS0)/N
  T_IH <- 1/mu_NT + 1/(mu_HI+mu_HS)
  n_ep <- 1 + mu_HI/mu_HS
  z <- lambda0*(1-epsilon*phi0)*n_ep*T_IH
  H0 <- z/(1+z)*N - I0
  
  init_state <- c("S" = N-I0-H0, "IA" = IA0, "IS" = IS0, "IT" = 0, "IM" = 0, "H"=H0) 
  ...
  mk_ode <- function(time, state, parameters) {
    ...
    dS <- -lambda*S + mu_T*IT + mu_HS*H
    dIA <- (1-epsilon)*(lambda*(S+H)+mu_HI*H) - mu_NT*IA
    dIS <- epsilon*(1-phi[time])*(lambda*(S+H)+mu_HI*H) - mu_NT*IS
    dIT <- epsilon*(phi[time]-m[time])*(lambda*(S+H)+mu_HI*H) - mu_T*IT
    dIM <- epsilon*m[time]*(lambda*(S+H)+mu_HI*H) - mu_T*IM
    dH <- -(lambda + mu_HI + mu_HS)*H + mu_NT*(IA+IS) + mu_T*IM 
    
    return(list(c(dS, dIA, dIS, dIT, dIM, dH)))
  }
